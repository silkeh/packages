...
class PackageDirectory(PullRequestCheck):
    _two_letter_dirs = ['py']
    _error = 'Package not correct directory'
    _level = Level.ERROR
...
class Pspec(PullRequestCheck):
    ...
    def _yml_file(package_dir: str) -> str:
        return os.path.join(package_dir, 'package.yml')

    @staticmethod
    def _xml_file(package_dir: str) -> str:
        return os.path.join(package_dir, 'pspec_x86_64.xml')
...
def files_from_ref(ref: str) -> List[str]:
    _git(['fetch'] + _ref_to_fetch(ref))

    return _git(['diff', '--name-only', '--diff-filter=d', ref]).split("\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--ref', type=str,
                        help='Optional reference to get changed files from, eg: origin/main..HEAD')
    parser.add_argument('filename', type=str, nargs="*",
                        help='Additional files to check')
    args = parser.parse_args()

    if args.ref:
        args.filename += files_from_ref(args.ref)
